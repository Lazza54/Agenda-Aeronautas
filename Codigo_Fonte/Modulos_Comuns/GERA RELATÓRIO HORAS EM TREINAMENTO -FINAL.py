...
# #            e gerar um relatório em um arquivo texto chamado 'rel_horas_tempo_de_treinamento.txt'
# UTILIZA O ARQUIVO 
...
# Filtrar onde 'Activity' esteja na lista_treinamento' 
...
# Filtrar o DataFrame para incluir apenas as linhas onde a coluna 'Activity' está na lista_treinamento
dados_filtrados = dados_iniciais[dados_iniciais['Activity'].isin(lista_treinamentos)]
...
dados = dados[(dados['Tempo Treinamento'].notnull()) & (dados['Tempo Treinamento'] != '') & (dados['Tempo Treinamento'] != '-')]
dados = dados[(dados['Tempo Treinamento Diurno'].notnull()) & (dados['Tempo Treinamento Diurno'] != '') & (dados['Tempo Treinamento Diurno'] != '-')]
dados = dados[(dados['Tempo Treinamento Noturno'].notnull()) & (dados['Tempo Treinamento Noturno'] != '') & (dados['Tempo Treinamento Noturno'] != '-')]
dados = dados[(dados['Tempo Treinamento Especial Diurno'].notnull()) & (dados['Tempo Treinamento Especial Diurno'] != '') & (dados['Tempo Treinamento Especial Diurno'] != '-')]
dados = dados[(dados['Tempo Treinamento Especial Noturno'].notnull()) & (dados['Tempo Treinamento Especial Noturno'] != '') & (dados['Tempo Treinamento Especial Noturno'] != '-')]

# CONVERTER AS COLUNAS Repouso, TJS, TJSD, TJSN, TJSE e TJSEN PARA timedelta
dados['Tempo Treinamento'] = pd.to_timedelta(dados['Tempo Treinamento'])
dados['Tempo Treinamento Diurno'] = pd.to_timedelta(dados['Tempo Treinamento Diurno'])
dados['Tempo Treinamento Noturno'] = pd.to_timedelta(dados['Tempo Treinamento Noturno'])
dados['Tempo Treinamento Especial Diurno'] = pd.to_timedelta(dados['Tempo Treinamento Especial Diurno'])
dados['Tempo Treinamento Especial Noturno'] = pd.to_timedelta(dados['Tempo Treinamento Especial Noturno'])
...
# Função para limpar e corrigir valores da coluna 'Tempo Treinamento '
...
def totalizar_por_semana(dados):
    ...
    colunas_tempo = ['Tempo Treinamento', 'Tempo Treinamento Diurno', 'Tempo Treinamento Noturno', 'Tempo Treinamento Especial Diurno', 'Tempo Treinamento Especial Noturno']
...
novo_nome = nome_arquivo.split(")")[0] + ") - REL_HORAS_DE_TREINAMENTO.txt"

with open(novo_nome, 'w') as arquivo:

    # centralizar o título
    titulo = 'RELATORIO TOTAL DE HORAS DE TREINAMENTO' 
    largura_total = 120
    ...
    arquivo.write('Activity'.ljust(15) + 'Checkin'.ljust(21) + 'Start'.ljust(14) + 'Dep'.ljust(5) + 'Arr'.ljust(12) + 'End'.ljust(20) + 'Checkout'.ljust(15) + 'Treinamento'.ljust(15) + '\n')
    arquivo.write(linha_simples + '\n')
    
    dados = dados.reset_index(drop=True)
    
    for linha in range(len(dados) - 1):
        ...
        # Converter 'Tempo Treinamento ' para o formato HH:MM
        tempo_treinamento_timedelta = dados['Tempo Treinamento'][linha]
        horas, resto = divmod(tempo_treinamento_timedelta.total_seconds(), 3600)
        minutos, _ = divmod(resto, 60)
        tempo_treinamento_formatada = f"{int(horas):02}:{int(minutos):02}"
        ...
        arquivo.write(dados['Activity'][linha].ljust(8) + 
                      ...
                      tempo_treinamento_formatada.ljust(15) + '\n')

        # imprimir os totais por semana

        if linha < len(dados) - 1:
            
            if pd.to_datetime(dados['Checkin'][linha]).isocalendar().week != pd.to_datetime(dados['Checkin'][linha + 1]).isocalendar().week:
                ...
                total_semana['Tempo Treinamento                        = '] = total_semana['Tempo Treinamento'].dt.components.hours + total_semana['Tempo Treinamento'].dt.components.days * 24
                
                total_semana['Tempo Treinamento Diurno           = '] = total_semana['Tempo Treinamento Diurno'].dt.components.hours + total_semana['Tempo Treinamento Diurno'].dt.components.days * 24
                total_semana['Tempo Treinamento Noturno          = '] = total_semana['Tempo Treinamento Noturno'].dt.components.hours + total_semana['Tempo Treinamento Noturno'].dt.components.days * 24
                total_semana['Tempo Treinamento Especial Diurno  = '] = total_semana['Tempo Treinamento Especial Diurno'].dt.components.hours + total_semana['Tempo Treinamento Especial Diurno'].dt.components.days * 24
                total_semana['Tempo Treinamento Especial Noturno = '] = total_semana['Tempo Treinamento Especial Noturno'].dt.components.hours + total_semana['Tempo Treinamento Especial Noturno'].dt.components.days * 24
                
                ###

                total_semana['Tempo Treinamento                        = '] = total_semana['Tempo Treinamento                        = '].astype(str).str.zfill(2) + ':' + total_semana['Tempo Treinamento'].dt.components.minutes.astype(str).str.zfill(2)
                
                total_semana['Tempo Treinamento Diurno           = '] = total_semana['Tempo Treinamento Diurno           = '].astype(str).str.zfill(2) + ':' + total_semana['Tempo Treinamento Diurno'].dt.components.minutes.astype(str).str.zfill(2)
                total_semana['Tempo Treinamento Noturno          = '] = total_semana['Tempo Treinamento Noturno          = '].astype(str).str.zfill(2) + ':' + total_semana['Tempo Treinamento Noturno'].dt.components.minutes.astype(str).str.zfill(2)
                total_semana['Tempo Treinamento Especial Diurno  = '] = total_semana['Tempo Treinamento Especial Diurno  = '].astype(str).str.zfill(2) + ':' + total_semana['Tempo Treinamento Especial Diurno'].dt.components.minutes.astype(str).str.zfill(2)
                total_semana['Tempo Treinamento Especial Noturno = '] = total_semana['Tempo Treinamento Especial Noturno = '].astype(str).str.zfill(2) + ':' + total_semana['Tempo Treinamento Especial Noturno'].dt.components.minutes.astype(str).str.zfill(2)
                ...
                if periodo_semanal in total_semana.index:
                    ...
                    valores = total_semana.loc[periodo_semanal, ['Tempo Treinamento                        = ', 'Tempo Treinamento Diurno           = ', 'Tempo Treinamento Noturno          = ', 'Tempo Treinamento Especial Diurno  = ', 'Tempo Treinamento Especial Noturno = ']]
                ...
                arquivo.write(titulo_centralizado + '\n')
                # alinhar a linha abaixo como cabeçalho
                arquivo.write('Activity'.ljust(15) + 'Checkin'.ljust(21) + 'Start'.ljust(14) + 'Dep'.ljust(5) + 'Arr'.ljust(12) + 'End'.ljust(20) + 'Checkout'.ljust(15) + 'Treinamento'.ljust(21) + '\n')
            ...
            if semana_atual in total_semana.index: # coloquei agora
                ...
                valores = total_semana.loc[semana_atual, ['Tempo Treinamento', 'Tempo Treinamento Diurno', 'Tempo Treinamento Noturno', 'Tempo Treinamento Especial Diurno', 'Tempo Treinamento Especial Noturno']]
                ...
                for coluna in valores.index:
                    ...
                    #linha_formatada = f"{coluna.ljust(45)} = : {valor_str.rjust(7)}"
    ...
    for linha in range(len(dados)):
        ...
        if linha < len(dados) - 1:
            if pd.to_datetime(dados['Checkin'][linha]).month != pd.to_datetime(dados['Checkin'][linha + 1]).month:
                ...
                total_mes['Tempo Treinamento                        = '] = total_mes['Tempo Treinamento'].dt.components.hours + total_mes['Tempo Treinamento'].dt.components.days * 24
                
                total_mes['Tempo Treinamento Diurno           = '] = total_mes['Tempo Treinamento Diurno'].dt.components.hours + total_mes['Tempo Treinamento Diurno'].dt.components.days * 24
                total_mes['Tempo Treinamento Noturno          = '] = total_mes['Tempo Treinamento Noturno'].dt.components.hours + total_mes['Tempo Treinamento Noturno'].dt.components.days * 24
                total_mes['Tempo Treinamento Especial Diurno  = '] = total_mes['Tempo Treinamento Especial Diurno'].dt.components.hours + total_mes['Tempo Treinamento Especial Diurno'].dt.components.days * 24
                total_mes['Tempo Treinamento Especial Noturno = '] = total_mes['Tempo Treinamento Especial Noturno'].dt.components.hours + total_mes['Tempo Treinamento Especial Noturno'].dt.components.days * 24
                
                ###

                total_mes['Tempo Treinamento                        = '] = total_mes['Tempo Treinamento                        = '].astype(str).str.zfill(2) + ':' + total_mes['Tempo Treinamento'].dt.components.minutes.astype(str).str.zfill(2)
                
                total_mes['Tempo Treinamento Diurno           = '] = total_mes['Tempo Treinamento Diurno           = '].astype(str).str.zfill(2) + ':' + total_mes['Tempo Treinamento Diurno'].dt.components.minutes.astype(str).str.zfill(2)
                total_mes['Tempo Treinamento Noturno          = '] = total_mes['Tempo Treinamento Noturno          = '].astype(str).str.zfill(2) + ':' + total_mes['Tempo Treinamento Noturno'].dt.components.minutes.astype(str).str.zfill(2)
                total_mes['Tempo Treinamento Especial Diurno  = '] = total_mes['Tempo Treinamento Especial Diurno  = '].astype(str).str.zfill(2) + ':' + total_mes['Tempo Treinamento Especial Diurno'].dt.components.minutes.astype(str).str.zfill(2)
                total_mes['Tempo Treinamento Especial Noturno = '] = total_mes['Tempo Treinamento Especial Noturno = '].astype(str).str.zfill(2) + ':' + total_mes['Tempo Treinamento Especial Noturno'].dt.components.minutes.astype(str).str.zfill(2)
                ...
                if periodo_mensal in total_mes.index:
                    # Acessar os valores específicos das colunas desejadas
                    valores = total_mes.loc[periodo_mensal, ['Tempo Treinamento                        = ', 'Tempo Treinamento Diurno           = ', 'Tempo Treinamento Noturno          = ', 'Tempo Treinamento Especial Diurno  = ', 'Tempo Treinamento Especial Noturno = ']]
