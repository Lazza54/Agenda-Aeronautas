...
# #            e gerar um relatório em um arquivo texto chamado 'rel_horas_tempo_de_plantão.txt'
# UTILIZA O ARQUIVO 
...
dados_iniciais = pd.read_csv(nome_arquivo, sep=',', encoding='utf-8', parse_dates=['Checkin', 'Start', 'End','Checkout'], dayfirst=True)
...
##### CARREGAR SIGLAS DE PLANTÃO E SOBREAVISO
...
# Filtrar onde 'Activity' está em lista_sobreavisos
dados_filtrados = dados_filtrados[dados_filtrados['Activity'].isin(lista_sobreavisos)]  

# CRIAR O DATAFRAME dados A PARTIR DO DATAFRAME dados_iniciais e com apenas as colunas relevantes
colunas_relevantes = [
    'Checkin',
    'Start',
    'Dep',
...
dados_filtrados = dados_filtrados[colunas_relevantes]
...
pprint(dados_filtrados)  

# Filtrar o DataFrame para excluir as linhas onde a coluna 'Tempos Plantaos' é nula, vazia ou igual a '-'
dados = dados[(dados['Tempo Plantao Diurno'].notnull()) & (dados['Tempo Plantao Diurno'] != '') & (dados['Tempo Plantao Diurno'] != '-')]
dados = dados[(dados['Tempo Plantao Noturno'].notnull()) & (dados['Tempo Plantao Noturno'] != '') & (dados['Tempo Plantao Noturno'] != '-')]
dados = dados[(dados['Tempo Plantao Especial Diurno'].notnull()) & (dados['Tempo Plantao Especial Diurno'] != '') & (dados['Tempo Plantao Especial Diurno'] != '-')]
dados = dados[(dados['Tempo Plantao Especial Noturno'].notnull()) & (dados['Tempo Plantao Especial Noturno'] != '') & (dados['Tempo Plantao Especial Noturno'] != '-')]

# CONVERTER AS COLUNAS Repouso, TJS, TJSD, TJSN, TJSE e TJSEN PARA timedelta
#dados['Tempo Plantao '] = pd.to_timedelta(dados['Tempo Plantao '])
dados['Tempo Plantao Diurno'] = pd.to_timedelta(dados['Tempo Plantao Diurno'])
dados['Tempo Plantao Noturno'] = pd.to_timedelta(dados['Tempo Plantao Noturno'])
dados['Tempo Plantao Especial Diurno'] = pd.to_timedelta(dados['Tempo Plantao Especial Diurno'])
dados['Tempo Plantao Especial Noturno'] = pd.to_timedelta(dados['Tempo Plantao Especial Noturno'])

pprint(dados, width=60, depth=2, compact=True)
...
# Função para limpar e corrigir valores da coluna 'Tempo Plantao '
...
def totalizar_por_semana(dados):
    ...
    colunas_tempo = ['Tempo Plantao Diurno', 'Tempo Plantao Noturno', 'Tempo Plantao Especial Diurno', 'Tempo Plantao Especial Noturno']
...
def totalizar_por_mes(dados):
    ...
    # Lista das colunas de tempo
    #colunas_tempo = ['Tempo Plantao ', 'Tempo Plantao Diurno', 'Tempo Plantao Noturno', 'Tempo Plantao Especial Diurno', 'Tempo Plantao Especial Noturno']
...
with open(novo_nome, 'w') as arquivo:
    ...
    for linha in range(len(dados)):
        ...
        # Converter 'Tempo Plantao ' para o formato HH:MM
        Plantao_timedelta = dados['Tempo Plantao Diurno'][linha]
        horas, resto = divmod(Plantao_timedelta.total_seconds(), 3600)
        minutos, _ = divmod(resto, 60)
        Plantao_formatada = f"{int(horas):02}:{int(minutos):02}"

        print(f"Processando linha {linha + 1}/{len(dados)}: {gerar_barra_progresso(linha + 1, len(dados))}", end='\r')
        ...
        if linha < len(dados) - 1:
            ...
            if pd.to_datetime(dados['Checkin'][linha]).week != pd.to_datetime(dados['Checkin'][linha + 1]).week:
                ...
                #total_semana['Repouso                        = '] = total_semana['Tempo Plantao '].dt.components.hours + total_semana['Tempo Plantao '].dt.components.days * 24
                
                total_semana['Tempo Plantao Diurno           = '] = total_semana['Tempo Plantao Diurno'].dt.components.hours + total_semana['Tempo Plantao Diurno'].dt.components.days * 24
                total_semana['Tempo Plantao Noturno          = '] = total_semana['Tempo Plantao Noturno'].dt.components.hours + total_semana['Tempo Plantao Noturno'].dt.components.days * 24
                total_semana['Tempo Plantao Especial Diurno  = '] = total_semana['Tempo Plantao Especial Diurno'].dt.components.hours + total_semana['Tempo Plantao Especial Diurno'].dt.components.days * 24
                total_semana['Tempo Plantao Especial Noturno = '] = total_semana['Tempo Plantao Especial Noturno'].dt.components.hours + total_semana['Tempo Plantao Especial Noturno'].dt.components.days * 24
                
                ###
                
                total_semana['Tempo Plantao Diurno           = '] = total_semana['Tempo Plantao Diurno           = '].astype(str).str.zfill(2) + ':' + total_semana['Tempo Plantao Diurno'].dt.components.minutes.astype(str).str.zfill(2)
                total_semana['Tempo Plantao Noturno          = '] = total_semana['Tempo Plantao Noturno          = '].astype(str).str.zfill(2) + ':' + total_semana['Tempo Plantao Noturno'].dt.components.minutes.astype(str).str.zfill(2)
                total_semana['Tempo Plantao Especial Diurno  = '] = total_semana['Tempo Plantao Especial Diurno  = '].astype(str).str.zfill(2) + ':' + total_semana['Tempo Plantao Especial Diurno'].dt.components.minutes.astype(str).str.zfill(2)
                total_semana['Tempo Plantao Especial Noturno = '] = total_semana['Tempo Plantao Especial Noturno = '].astype(str).str.zfill(2) + ':' + total_semana['Tempo Plantao Especial Noturno'].dt.components.minutes.astype(str).str.zfill(2)
                ...
                if periodo_semanal in total_semana.index:
                    ...
                    #valores = total_semana.loc[periodo_semanal, ['Repouso                        = ', 'Tempo Plantao Diurno           = ', 'Tempo Plantao Noturno          = ', 'Tempo Plantao Especial Diurno  = ', 'Tempo Plantao Especial Noturno = ']]
                    
                    valores = total_semana.loc[periodo_semanal, ['Tempo Plantao Diurno           = ', 'Tempo Plantao Noturno          = ', 'Tempo Plantao Especial Diurno  = ', 'Tempo Plantao Especial Noturno = ']]                    
            ...
            if semana_atual in total_semana.index: # coloquei agora
                ...
                #valores = total_semana.loc[semana_atual, ['Tempo Plantao ', 'Tempo Plantao Diurno', 'Tempo Plantao Noturno', 'Tempo Plantao Especial Diurno', 'Tempo Plantao Especial Noturno']]
                
                valores = total_semana.loc[semana_atual, ['Tempo Plantao Diurno', 'Tempo Plantao Noturno', 'Tempo Plantao Especial Diurno', 'Tempo Plantao Especial Noturno']]
    ...
    for linha in range(len(dados)):
        ...
        if linha < len(dados) - 1:
            if pd.to_datetime(dados['Checkin'][linha]).month != pd.to_datetime(dados['Checkin'][linha + 1]).month:
                ...
                #total_mes['Repouso                        = '] = total_mes['Tempo Plantao '].dt.components.hours + total_mes['Tempo Plantao '].dt.components.days * 24
                
                total_mes['Tempo Plantao Diurno           = '] = total_mes['Tempo Plantao Diurno'].dt.components.hours + total_mes['Tempo Plantao Diurno'].dt.components.days * 24
                total_mes['Tempo Plantao Noturno          = '] = total_mes['Tempo Plantao Noturno'].dt.components.hours + total_mes['Tempo Plantao Noturno'].dt.components.days * 24
                total_mes['Tempo Plantao Especial Diurno  = '] = total_mes['Tempo Plantao Especial Diurno'].dt.components.hours + total_mes['Tempo Plantao Especial Diurno'].dt.components.days * 24
                total_mes['Tempo Plantao Especial Noturno = '] = total_mes['Tempo Plantao Especial Noturno'].dt.components.hours + total_mes['Tempo Plantao Especial Noturno'].dt.components.days * 24
                
                total_mes['Tempo Plantao Diurno           = '] = total_mes['Tempo Plantao Diurno           = '].astype(str).str.zfill(2) + ':' + total_mes['Tempo Plantao Diurno'].dt.components.minutes.astype(str).str.zfill(2)
                total_mes['Tempo Plantao Noturno          = '] = total_mes['Tempo Plantao Noturno          = '].astype(str).str.zfill(2) + ':' + total_mes['Tempo Plantao Noturno'].dt.components.minutes.astype(str).str.zfill(2)
                total_mes['Tempo Plantao Especial Diurno  = '] = total_mes['Tempo Plantao Especial Diurno  = '].astype(str).str.zfill(2) + ':' + total_mes['Tempo Plantao Especial Diurno'].dt.components.minutes.astype(str).str.zfill(2)
                total_mes['Tempo Plantao Especial Noturno = '] = total_mes['Tempo Plantao Especial Noturno = '].astype(str).str.zfill(2) + ':' + total_mes['Tempo Plantao Especial Noturno'].dt.components.minutes.astype(str).str.zfill(2)
                ...
                if periodo_mensal in total_mes.index:
                    # Acessar os valores específicos das colunas desejadas
                    #valores = total_mes.loc[periodo_mensal, ['Repouso                        = ', 'Tempo Plantao Diurno           = ', 'Tempo Plantao Noturno          = ', 'Tempo Plantao Especial Diurno  = ', 'Tempo Plantao Especial Noturno = ']]
                    valores = total_mes.loc[periodo_mensal, ['Tempo Plantao Diurno           = ', 'Tempo Plantao Noturno          = ', 'Tempo Plantao Especial Diurno  = ', 'Tempo Plantao Especial Noturno = ']]
    ...
    if periodo_mensal in total_mes.index:
        ...
        # Iterar sobre as colunas e escrever cada valor no arquivo
        for coluna in valores.index:
            valor_str = f"{coluna}: {valores[coluna]}"
            arquivo.write(valor_str.ljust(105) + '\n')
    else:
        print(f"Período mensal {periodo_mensal} não encontrado no índice de 'total_mes'.")
