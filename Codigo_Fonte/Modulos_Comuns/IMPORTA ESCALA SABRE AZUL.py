...
def _times_with_plus_and_pos(line: str) -> List[Tuple[str, bool, int, int]]:
    ...
    Detecta '00:33 +1' mesmo com espaços variados.
    """
    out = []
...
def _token_right_after(line: str, pos: int) -> str:
    """
    CORRIGIDO: pega o 1º token ÚTIL após 'pos', pulando '+1' e separadores.
    Preferência por IATA (AAA). Se não houver IATA, pega token alfanumérico.
    Ex.: '00:00 - 00:00 +1 VCP' -> retorna 'VCP'
    """
    idx = pos
    # pula repetições de: espaços, +1, e separadores simples
    while True:
        m_skip = re.match(r"\s*(\+1|[-–—/|])\s*", line[idx:])
        if not m_skip:
            break
        # se encontrou '+1' ou separador, avança e continua
        idx += m_skip.end()
    ...
    # senão, pega token alfanumérico curto
...
def pdf_para_csv(pdf_path: str) -> str:
    ...
    for ln in lines:
        ...
        if lnl.startswith("off"):
            ...
            dep = arr = ""
            if tms:
                token = _token_right_after(ln, tms[-1][3])  # agora pula '+1' e pega VCP/...
                if token:
                    dep = token; arr = token
            if current_date:
                start_dt = datetime.strptime(current_date + " 00:00", "%d/%m/%Y %H:%M")
                end_dt   = start_dt + timedelta(hours=24)
                row = {
                    "Activity": activity,
                    "Checkin":  start_dt.strftime("%d/%m/%Y %H:%M"),  # provisório; será trocado se houver pending
                    "Start":    start_dt.strftime("%d/%m/%Y %H:%M"),
                    "Dep":      dep,
                    "Arr":      arr,
                    "End":      end_dt.strftime("%d/%m/%Y %H:%M"),
                    "Checkout": end_dt.strftime("%d/%m/%Y %H:%M"),
                    "AcVer":    "",
                    "DD":       "",
                    "CAT":      _cat_token(ln),
                    "Crew":     "",
        ...
        if lnl.startswith("ground") or " ground" in lnl:
            ...
            row = {
                "Activity": activity,
                "Checkin":  start,   # provisório; será trocado se houver pending
                "Start":    start,
                "Dep":      dep,
                "Arr":      arr,
                "End":      end,
        ...
        if act and current_date:
            ...
            row = {
                "Activity": act,
                "Checkin":  "",  # provisório; será trocado se houver pending
                "Start":    start,
                "Dep":      dep,
                "Arr":      arr,
                "End":      end,
