...
#            a partir de um arquivo CSV com as colunas Checkin, Jornada, TJS, TJSD, TJSN, TJSE e TJSEN
#            e gerar um relatório em um arquivo texto chamado 'rel_horas_tempo_jornadasdetrabalho.txt'
# UTILIZA O ARQUIVO JORNADAS
...
# Filtrar o DataFrame para excluir as linhas onde a coluna 'Jornada' é nula, vazia ou igual a '-'
dados = dados[(dados['Jornada'].notnull()) & (dados['Jornada'] != '') & (dados['Jornada'] != '-')]

# CONVERTER AS COLUNAS Jornada, TJS, TJSD, TJSN, TJSE e TJSEN PARA timedelta
dados['Jornada'] = pd.to_timedelta(dados['Jornada'])
dados['Tempo Jornada Diurno'] = pd.to_timedelta(dados['Tempo Jornada Diurno'])
dados['Tempo Jornada Noturno'] = pd.to_timedelta(dados['Tempo Jornada Noturno'])
dados['Tempo Jornada Especial Diurno'] = pd.to_timedelta(dados['Tempo Jornada Especial Diurno'])
dados['Tempo Jornada Especial Noturno'] = pd.to_timedelta(dados['Tempo Jornada Especial Noturno'])
...
# Função para limpar e corrigir valores da coluna 'Jornada'
def corrigir_jornada(valor):
    try:
...
def totalizar_por_semana(dados):
    ...
    colunas_tempo = ['Jornada', 'Tempo Jornada Diurno', 'Tempo Jornada Noturno', 'Tempo Jornada Especial Diurno', 'Tempo Jornada Especial Noturno']
...
novo_nome = nome_arquivo.split(")")[0] + ") - REL_HORAS_TEMPO_JORNADA_DE_TRABALHO.txt"

print(f"Gerando relatório em {novo_nome}...")

with open(novo_nome, 'w') as arquivo:
    # centralizar o título
    titulo = 'EXTRATO DE HORAS DAS JORNADAS DE TRABALHO SEMANAIS E MENSAIS' 
    largura_total = 120
    titulo_centralizado = titulo.center(largura_total)
    arquivo.write(titulo_centralizado + '\n')
    
    # alinhar a linha abaixo como cabeçalho
    arquivo.write('Activity'.ljust(15) + 'Checkin'.ljust(21) + 'Start'.ljust(14) + 'Dep'.ljust(5) + 'Arr'.ljust(12) + 'End'.ljust(20) + 'Checkout'.ljust(15) + 'Jornada'.ljust(15) + '\n')

    arquivo.write(linha_simples + '\n')

    for linha in range(len(dados)):

        #  imprimir os dados da linha atual com as seguintes colunas 'activity' 'checkin' 'start' 'dep' 'arr' 'end' 'checkout' 'jornada, antes de pular para a próxima semana ou mês imprimir os totais

        # Converter 'Jornada' para o formato HH:MM
        jornada_timedelta = dados['Jornada'][linha]
        horas, resto = divmod(jornada_timedelta.total_seconds(), 3600)
        minutos, _ = divmod(resto, 60)
        jornada_formatada = f"{int(horas):02}:{int(minutos):02}"
        ...
        if linha < len(dados) - 1:
            ...
            if pd.to_datetime(dados['Checkin'][linha]).week != pd.to_datetime(dados['Checkin'][linha + 1]).week:
                ...
                ### TENTATIVA DE ALTERAR jornada_hhmm PARA Jornada = 
                total_semana['Jornada                        = '] = total_semana['Jornada'].dt.components.hours + total_semana['Jornada'].dt.components.days * 24
                total_semana['Tempo Jornada Diurno           = '] = total_semana['Tempo Jornada Diurno'].dt.components.hours + total_semana['Tempo Jornada Diurno'].dt.components.days * 24
                total_semana['Tempo Jornada Noturno          = '] = total_semana['Tempo Jornada Noturno'].dt.components.hours + total_semana['Tempo Jornada Noturno'].dt.components.days * 24
                total_semana['Tempo Jornada Especial Diurno  = '] = total_semana['Tempo Jornada Especial Diurno'].dt.components.hours + total_semana['Tempo Jornada Especial Diurno'].dt.components.days * 24
                total_semana['Tempo Jornada Especial Noturno = '] = total_semana['Tempo Jornada Especial Noturno'].dt.components.hours + total_semana['Tempo Jornada Especial Noturno'].dt.components.days * 24
                
                ###

                total_semana['Jornada                        = '] = total_semana['Jornada                        = '].astype(str).str.zfill(2) + ':' + total_semana['Jornada'].dt.components.minutes.astype(str).str.zfill(2)
                total_semana['Tempo Jornada Diurno           = '] = total_semana['Tempo Jornada Diurno           = '].astype(str).str.zfill(2) + ':' + total_semana['Tempo Jornada Diurno'].dt.components.minutes.astype(str).str.zfill(2)
                total_semana['Tempo Jornada Noturno          = '] = total_semana['Tempo Jornada Noturno          = '].astype(str).str.zfill(2) + ':' + total_semana['Tempo Jornada Noturno'].dt.components.minutes.astype(str).str.zfill(2)
                total_semana['Tempo Jornada Especial Diurno  = '] = total_semana['Tempo Jornada Especial Diurno  = '].astype(str).str.zfill(2) + ':' + total_semana['Tempo Jornada Especial Diurno'].dt.components.minutes.astype(str).str.zfill(2)
                total_semana['Tempo Jornada Especial Noturno = '] = total_semana['Tempo Jornada Especial Noturno = '].astype(str).str.zfill(2) + ':' + total_semana['Tempo Jornada Especial Noturno'].dt.components.minutes.astype(str).str.zfill(2)
                ...
                if periodo_semanal in total_semana.index:
                    ...
                    valores = total_semana.loc[periodo_semanal, ['Jornada                        = ', 'Tempo Jornada Diurno           = ', 'Tempo Jornada Noturno          = ', 'Tempo Jornada Especial Diurno  = ', 'Tempo Jornada Especial Noturno = ']]
                ...
                arquivo.write(titulo_centralizado + '\n')
                # alinhar a linha abaixo como cabeçalho
                arquivo.write('Activity'.ljust(15) + 'Checkin'.ljust(21) + 'Start'.ljust(14) + 'Dep'.ljust(5) + 'Arr'.ljust(12) + 'End'.ljust(20) + 'Checkout'.ljust(15) + 'Jornada'.ljust(21) + '\n')
            ...
            if semana_atual in total_semana.index: # coloquei agora
                ...
                valores = total_semana.loc[semana_atual, ['Jornada', 'Tempo Jornada Diurno', 'Tempo Jornada Noturno', 'Tempo Jornada Especial Diurno', 'Tempo Jornada Especial Noturno']]
    ...
    for linha in range(len(dados)):
        ...
        if linha < len(dados) - 1:
            if pd.to_datetime(dados['Checkin'][linha]).month != pd.to_datetime(dados['Checkin'][linha + 1]).month:
                ...
                total_mes['Jornada                        = '] = total_mes['Jornada'].dt.components.hours + total_mes['Jornada'].dt.components.days * 24
                total_mes['Tempo Jornada Diurno           = '] = total_mes['Tempo Jornada Diurno'].dt.components.hours + total_mes['Tempo Jornada Diurno'].dt.components.days * 24
                total_mes['Tempo Jornada Noturno          = '] = total_mes['Tempo Jornada Noturno'].dt.components.hours + total_mes['Tempo Jornada Noturno'].dt.components.days * 24
                total_mes['Tempo Jornada Especial Diurno  = '] = total_mes['Tempo Jornada Especial Diurno'].dt.components.hours + total_mes['Tempo Jornada Especial Diurno'].dt.components.days * 24
                total_mes['Tempo Jornada Especial Noturno = '] = total_mes['Tempo Jornada Especial Noturno'].dt.components.hours + total_mes['Tempo Jornada Especial Noturno'].dt.components.days * 24
                
                ###

                total_mes['Jornada                        = '] = total_mes['Jornada                        = '].astype(str).str.zfill(2) + ':' + total_mes['Jornada'].dt.components.minutes.astype(str).str.zfill(2)
                total_mes['Tempo Jornada Diurno           = '] = total_mes['Tempo Jornada Diurno           = '].astype(str).str.zfill(2) + ':' + total_mes['Tempo Jornada Diurno'].dt.components.minutes.astype(str).str.zfill(2)
                total_mes['Tempo Jornada Noturno          = '] = total_mes['Tempo Jornada Noturno          = '].astype(str).str.zfill(2) + ':' + total_mes['Tempo Jornada Noturno'].dt.components.minutes.astype(str).str.zfill(2)
                total_mes['Tempo Jornada Especial Diurno  = '] = total_mes['Tempo Jornada Especial Diurno  = '].astype(str).str.zfill(2) + ':' + total_mes['Tempo Jornada Especial Diurno'].dt.components.minutes.astype(str).str.zfill(2)
                total_mes['Tempo Jornada Especial Noturno = '] = total_mes['Tempo Jornada Especial Noturno = '].astype(str).str.zfill(2) + ':' + total_mes['Tempo Jornada Especial Noturno'].dt.components.minutes.astype(str).str.zfill(2)
                ...
                if periodo_mensal in total_mes.index:
                    # Acessar os valores específicos das colunas desejadas
                    valores = total_mes.loc[periodo_mensal, ['Jornada                        = ', 'Tempo Jornada Diurno           = ', 'Tempo Jornada Noturno          = ', 'Tempo Jornada Especial Diurno  = ', 'Tempo Jornada Especial Noturno = ']]
